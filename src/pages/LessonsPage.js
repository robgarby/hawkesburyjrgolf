export const lessonsPage = {
  "id": "lessons",
  "title": {
    "en": "Lessons",
    "fr": "Leçons"
  },
  "eyebrow": {
    "en": "Junior instruction",
    "fr": "Enseignement junior"
  },
  "heading": {
    "en": "Lessons, camp instruction, and free volunteer-coach lesson requests.",
    "fr": "Leçons, camp d’enseignement et demandes de leçons gratuites avec des entraîneurs bénévoles."
  },
  "intro": {
    "en": "Boys and girls ages 9-17 can learn through group instruction and free volunteer-coach lessons made available in the same My Account area used by both Member and Community players. Lessons with Hawkesbury Golf and Curling Club professionals are booked through the club and are not free unless they are listed in the Lessons area of My Account.",
    "fr": "Les filles et garçons de 9 à 17 ans peuvent apprendre grâce aux leçons de groupe et aux leçons gratuites avec des entraîneurs bénévoles offertes dans le même espace Mon compte utilisé par les membres et les joueurs communautaires. Les leçons avec les professionnels du Hawkesbury Golf and Curling Club se réservent auprès du club et ne sont pas gratuites, sauf si elles sont affichées dans la section Leçons de Mon compte."
  },
  "image": "/images/putting-green.jpeg",
  "cta": {
    "en": "Log in",
    "fr": "Se connecter"
  },
  "seo": {
    "en": {
      "title": "Junior Golf Lessons in Hawkesbury",
      "description": "Junior golf lessons in Hawkesbury for boys and girls ages 9-17, including Monday Morning Golf Camp and free volunteer-coach junior lesson requests in My Account."
    },
    "fr": {
      "title": "Leçons de golf junior à Hawkesbury",
      "description": "Leçons de golf junior à Hawkesbury pour les filles et garçons de 9 à 17 ans, incluant le camp du lundi matin et les demandes de leçons gratuites avec des entraîneurs bénévoles dans Mon compte."
    }
  },
  "ctaRoute": "login",
  "layout": "two-column",
  "sections": [
    {
      "title": {
        "en": "Monday Morning Golf Camp",
        "fr": "Camp de golf du lundi matin"
      },
      "text": {
        "en": "All Members of Hawkesbury Golf and Curling Club are automatically registered in the Monday Morning Hawkesbury Junior Group Lesson Program, offered during the summer months.",
        "fr": "Tous les membres du Hawkesbury Golf and Curling Club sont automatiquement inscrits au programme de leçons de groupe junior du lundi matin, offert pendant les mois d'été."
      },
      "bullets": {
        "en": [
          "Group lesson program for Members of Hawkesbury Golf and Curling Club",
          "Designed to build fundamentals, confidence, and comfort around the game",
          "Community Members can contact Hawkesbury Golf and Curling Club about the program"
        ],
        "fr": [
          "Programme de leçons de groupe pour les membres du Hawkesbury Golf and Curling Club",
          "Conçu pour développer les bases, la confiance et l’aisance autour du golf",
          "Les membres communautaires peuvent communiquer avec le Hawkesbury Golf and Curling Club au sujet du programme"
        ]
      },
      "links": [
        {
          "route": "login",
          "label": {
            "en": "Program login",
            "fr": "Connexion programme"
          }
        },
        {
          "url": "https://www.hawkesburygolfandcurling.com",
          "label": {
            "en": "Visit HGCC website",
            "fr": "Visiter le site du HGCC"
          }
        }
      ]
    },
    {
      "title": {
        "en": "Free Volunteer-Coach Lessons",
        "fr": "Leçons gratuites avec entraîneurs bénévoles"
      },
      "text": {
        "en": "Volunteer coaches are willing to give free lessons to juniors. These free lessons are available in the same My Account area for both Member and Community players, where juniors can see available times, request single or group lessons, and view booked lessons once a teacher or admin accepts.",
        "fr": "Des entraîneurs bénévoles sont prêts à offrir des leçons gratuites aux juniors. Ces leçons gratuites sont offertes dans le même espace Mon compte pour les membres et les joueurs communautaires, où les juniors peuvent voir les heures disponibles, demander des leçons individuelles ou de groupe et consulter les leçons réservées lorsqu’un enseignant ou admin accepte."
      },
      "bullets": {
        "en": [
          "Only lessons made available in My Account are free to juniors",
          "Free lessons are provided by volunteer coaches",
          "To book a lesson with the club professionals, contact Hawkesbury Golf and Curling Club. Professional lessons are not free unless they are listed in the Lessons area of My Account.",
          "Request a single lesson or a group lesson with a maximum number of students",
          "Join available lesson times when space is open",
          "Booked lessons move into the Booked Lessons area for the student and teacher",
          "Multiple free lessons may be booked when space is available",
          "Community Program updates may include sponsored golf events at the course",
          "Community Members can also use their account to find other juniors to play with",
          "Open to boys and girls ages 9-17",
          "Not available as adult golf lessons",
          "Lessons can be <strong>given in French</strong> as there are several French-speaking volunteers. This must be indicated at the time of booking to ensure proper coach selection."
        ],
        "fr": [
          "Seules les leçons offertes dans Mon compte sont gratuites pour les juniors",
          "Les leçons gratuites sont données par des entraîneurs bénévoles",
          "Pour réserver une leçon avec les professionnels du club, communiquez avec le Hawkesbury Golf and Curling Club. Les leçons professionnelles ne sont pas gratuites, sauf si elles sont affichées dans la section Leçons de Mon compte.",
          "Demander une leçon individuelle ou une leçon de groupe avec un nombre maximum d’élèves",
          "Joindre les heures de leçon disponibles lorsqu’il reste des places",
          "Les leçons réservées passent dans la section Leçons réservées pour l’élève et l’enseignant",
          "Plusieurs leçons gratuites peuvent être réservées lorsque des places sont disponibles",
          "Les nouvelles du programme communautaire peuvent inclure des événements de golf commandités au club",
          "Les membres communautaires peuvent aussi utiliser leur compte pour trouver d’autres juniors avec qui jouer",
          "Ouvert aux filles et garçons de 9 à 17 ans",
          "Non offert comme leçons de golf pour adultes",
          "Les leçons peuvent être <strong>données en français</strong> car plusieurs bénévoles parlent français. Cela doit être indiqué lors de la réservation afin d’assurer la sélection d’un entraîneur approprié."
        ]
      },
      "links": [
        {
          "route": "login",
          "label": {
            "en": "Create account / Log in",
            "fr": "Portail de connexion"
          }
        },
        {
          "route": "login",
          "label": {
            "en": "Community account",
            "fr": "Compte communautaire"
          }
        }
      ]
    }
  ]
}
